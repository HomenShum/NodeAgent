...
const source = 'D:/VSCode Projects/cafecorner_nodebench/nodebench_ai4/.portfolio-worktrees/nodeagent-current-consumer-20260905';
const require=createRequire(path.join(source,'package.json'));
const {chromium}=require('playwright');
const {createServer}=await import(pathToFileURL(path.join(source,'node_modules/vite/dist/node/index.js')).href);
...
async function activateRetry(retry, keyboard, cell, phase) {
  if (keyboard) { await retry.focus(); await page.keyboard.press('Enter'); await page.keyboard.press('Enter'); (cell.keyboardActivations ??= []).push({phase, key:'Enter', count:2, method:'Two native key activations; the first may move focus to the existing empty composer.'}); return; }
...
try {
  ...
  for (kind of surfaces) {
    ...
    for (const width of [390]) {
      ...
      cell.console=[];cell.failedRequests=[];page.on('console', message=>{if(cell.console.length<200&&['error','warning'].includes(message.type()))cell.console.push({type:message.type(),text:message.text(),location:message.location()});});page.on('requestfailed',request=>{if(cell.failedRequests.length<200)cell.failedRequests.push({url:request.url(),failure:request.failure()});});
      ...
      await send(question); await page.waitForFunction(({tool})=>document.querySelectorAll(tool).length===6 && document.querySelectorAll(tool)[4].getAttribute('data-running')==='false' && document.querySelectorAll(tool)[5].getAttribute('data-running')==='true',{tool});
      const partialBefore=await page.locator(tool).nth(4).innerText();const graphBeforeStop=await graph();
      ...
      await page.waitForTimeout(750); check(await page.locator(bubble).last().innerText() === stoppedText, kind + ' stopped response remains stable ' + width); assert.deepEqual(await graph(), stoppedGraph); assert.deepEqual(stoppedGraph, graphBeforeStop);
      check(await page.locator(tool).nth(4).innerText()===partialBefore,kind+' Stop preserves the current response first completed tool result');check(await page.locator(tool).nth(4).getAttribute('data-interrupted')==='false',kind+' completed tool does not acquire stopped status');
      ...
      check(await page.locator(bubble).first().innerText()===earlier,kind+' late-Stop retry preserves earlier response');await snapshot(kind+'-late-stop-recovered-'+width);await ctx.close();continue;
